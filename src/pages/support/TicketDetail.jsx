      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white border-r border-gray-100 shadow-xl">
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-8 bg-[#F8FAFC]/50">
          ...
          {ticket.messages.map((msg) => {
            const isMe = Number(msg.user_id) === Number(localStorage.getItem('user_id')); // Agent's own message
            ...
            return (
              <div key={msg.id} className={`flex space-x-4 ${isCustomer ? '' : 'flex-row-reverse space-x-reverse'}`}>
                ...
                <div className={`p-6 rounded-3xl shadow-sm max-w-2xl ${
                  ...
                  {msg.attachment && (
                    <div className={`mt-4 pt-4 border-t ${isCustomer ? 'border-gray-50' : isNote ? 'border-yellow-200' : 'border-white/10'}`}>
                      <a 
                        href={getAttachmentUrl(msg.attachment)} 
                        target="_blank" 
                        rel="noopener noreferrer"
                        className={`inline-flex items-center p-3 rounded-2xl border transition-all group ${
                          isCustomer ? 'bg-gray-50 border-gray-100 hover:border-[#F97316]/30 hover:bg-white' : 
                          isNote ? 'bg-yellow-100/50 border-yellow-200 hover:bg-yellow-100 text-yellow-900' :
                          'bg-white/10 border-white/10 hover:bg-white/20'
                        }`}
                      >
                        <Paperclip className={`w-4 h-4 mr-2 ${isCustomer ? 'text-gray-400 group-hover:text-[#F97316]' : 'text-white'}`} />
                        <span className={`text-xs font-bold truncate max-w-[150px] ${isCustomer ? 'text-gray-600' : 'text-white'}`}>
            ...
            <div className="relative">
              {attachment && (
                <div className="absolute bottom-full left-0 mb-4 bg-white p-3 rounded-2xl shadow-xl border border-gray-100 flex items-center animate-in slide-in-from-bottom-2">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center mr-3 ${isInternal ? 'bg-yellow-100 text-yellow-600' : 'bg-[#F97316]/10 text-[#F97316]'}`}>
              ...
              <div className="absolute right-3 bottom-3 flex items-center space-x-2">
                ...
                <button 
                  ...
                  className={`p-3 rounded-xl transition-colors ${attachment ? 'bg-orange-100 text-[#F97316]' : 'text-gray-400 hover:bg-gray-100'}`}
                >
                  <Paperclip className="w-5 h-5" />
                </button>
                <button
